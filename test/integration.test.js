...
test.before(async () => {
  ...
  webServer = staticServer(); webServer.listen(PORT_WEB, HOST); webServer.unref();
  openRouterServer = openRouterMock(); openRouterServer.listen(PORT_OPENROUTER, HOST); openRouterServer.unref();
  geminiServer = geminiMock(); geminiServer.listen(PORT_GEMINI, HOST); geminiServer.unref();
...
skip("integration: deadline reminder snoozes for a chosen horizon", async () => {
  ...
  await new Promise((r) => web2.listen(WEB2, HOST, r));
...
// Cycle #242 — smoking-gun cards prefill the Ask panel.
skip("integration: smoking-gun cards ask about the sentence", async () => {
  const WEB2 = 4341;
  const web2 = staticServer();
  await new Promise((r) => web2.listen(WEB2, HOST, r));
  const ctx = await browser.newContext();
  ...
  try {
    const doc = "Lessee shall indemnify the landlord in perpetuity and waive all claims arising from the lease.";
    await page.goto(`http://127.0.0.1:${WEB2}/analyze.html`, { waitUntil: "networkidle" });
    await page.evaluate((d) => { document.getElementById("docInput").value = d; }, doc);
    await page.click("#analyzeBtn");
    ...
    assert.match(askState.value, /^What does this smoking-gun sentence mean: "/,
      "the Ask panel must be prefilled with the sentence question");
...
// Cycle #244 — exposure cards prefill the Ask panel.
skip("integration: exposure cards ask about the exposure", async () => {
  const WEB2 = 4341;
  const web2 = staticServer();
  await new Promise((r) => web2.listen(WEB2, HOST, r));
  const ctx = await browser.newContext();
  ...
  try {
    const doc = "The tenant shall be liable for $500,000 in damages and the company shall indemnify all claims without limit.";
    await page.goto(`http://127.0.0.1:${WEB2}/analyze.html`, { waitUntil: "networkidle" });
    await page.evaluate((d) => { document.getElementById("docInput").value = d; }, doc);
    await page.click("#analyzeBtn");
    ...
    assert.match(askState.value, /^What does this exposure mean: "/,
      "the Ask panel must be prefilled with the exposure question");
    assert.ok(askState.value.includes(sentence.slice(0, 40)),
      "the prefilled question must carry the flagged sentence");
    assert.equal(askState.disabled, false, "the Ask input must be enabled");
    assert.equal(errors.length, 0, `zero console errors, got: ${errors.join(" | ")}`);
...
// Cycle #252 — the signing checklist downloads as a CSV with progress.
skip("integration: signing checklist downloads a CSV tracker file", async () => {
  const WEB2 = 4341;
  const web2 = staticServer();
  await new Promise((r) => web2.listen(WEB2, HOST, r));
  const ctx = await browser.newContext({ acceptDownloads: true });
  ...
  try {
    const doc = "A notary must witness the signature. The parties waive trial by jury and agree to binding arbitration. This Agreement may be executed in counterparts.";
    await page.goto(`http://127.0.0.1:${WEB2}/analyze.html`, { waitUntil: "networkidle" });
    await page.evaluate((d) => { document.getElementById("docInput").value = d; }, doc);
    await page.click("#analyzeBtn");
    ...
    const dlPath = await download.path();
    const content = fs.readFileSync(dlPath, "utf8");
    assert.match(download.suggestedFilename(), /^cleardoc-signing-\d{4}-\d{2}-\d{2}\.csv$/,
      "the CSV must download as cleardoc-signing-<date>.csv");
    assert.equal(content.charCodeAt(0), 0xFEFF, "the CSV must start with a UTF-8 BOM");
    const lines = content.slice(1).split("\n");
    assert.match(lines[0], /Signing checklist/, "the CSV must open with a metadata row");
    assert.match(lines[1], /Status.*Who.*Action.*Hint/, "the CSV must carry the column header");
    const dataRows = lines.slice(2).filter((l) => l.trim().length > 0);
...
// Cycle #312 — the compare panel downloads its comparison as Markdown.
skip("integration: compare panel downloads a Markdown comparison file", async () => {
  const WEB2 = 4341;
  const web2 = staticServer();
  await new Promise((r) => web2.listen(WEB2, HOST, r));
  const ctx = await browser.newContext({ acceptDownloads: true });
  ...
  try {
    const docA = "Lessee shall indemnify in perpetuity. Auto-renews for successive terms unless cancelled in writing.";
    const docB = "The parties agree to binding arbitration and waive trial by jury.";
    await page.goto(`http://127.0.0.1:${WEB2}/analyze.html`, { waitUntil: "networkidle" });
    await page.evaluate((d) => { document.getElementById("docInput").value = d; }, docA);
    ...
    assert.match(content, /\| Metric \| Original \| Compare \|/, "the file must carry the comparison table header");
    assert.match(content, /\| type \|/, "the file must include the type metric row");
    assert.match(content, /_Generated by ClearDoc/, "the file must carry the informational-only footer");
    assert.equal(errors.length, 0, `zero console errors, got: ${errors.join(" | ")}`);
  } finally {
    await page.close();
    await ctx.close();
