...
skip("analyzer: pre-sign brief copies the decision, risks, deadlines, and next steps", async () => {
  ...
  assert.match(brief, /http.*#share=/, "pre-sign brief must include a share link");
...
skip("analyzer: contract care plan copies renewal + upcoming deadlines", async () => {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/analyze.html`, { waitUntil: "networkidle" });
  await page.click("#analyzeBtn");
  await page.waitForSelector("#careBlock:not([hidden])", { timeout: 8000 });
  const plan = await page.evaluate(async () => {
    ...
    document.getElementById("careCopyBtn").click();
    await new Promise(r => setTimeout(r, 400));
    if (navigator.clipboard && orig) navigator.clipboard.writeText = orig;
    return captured;
  });
  assert.ok(plan, "care plan should copy something");
  assert.match(plan, /CLEARDOC CONTRACT CARE PLAN/, "plan must carry a clear header");
  assert.match(plan, /http.*#share=/, "care plan must include a share link");
  assert.match(plan, /_Generated by ClearDoc — informational only, not legal advice\._/, "plan must carry the disclaimer footer");
  const hasObligations = await page.evaluate(() => !!document.querySelector("#careGrid .care-tag[data-label='Obligation to fulfill']"));
  if(hasObligations){
    assert.match(plan, /Obligation:/, "plan must label obligations when present");
  }
  await page.close();
});
...
skip("analyzer: care plan next-dates digest copies dated items", async () => {
  ...
  page.on("pageerror", (e) => errors.push(String(e)));
  await page.addInitScript(() => {
    window.__copiedCareDates = null;
    try {
      Object.defineProperty(navigator, "clipboard", {
        configurable: true,
        value: {
          writeText: async (txt) => { window.__copiedCareDates = txt; },
          write: async () => {},
        },
      });
    } catch (_) {
      try { navigator.clipboard = { writeText: async (txt) => { window.__copiedCareDates = txt; }, write: async () => {} }; } catch (_2) {}
    }
  });
  try {
    await page.goto(`http://127.0.0.1:${PORT}/analyze.html`, { waitUntil: "networkidle" });
    await page.click(".qf[data-fill]:first-of-type");
    await page.click("#analyzeBtn");
    await page.waitForSelector("#careBlock:not([hidden]) #careDatesBtn", { timeout: 8000 });
    await page.click("#careDatesBtn");
    await page.waitForFunction(() => window.__copiedCareDates && window.__copiedCareDates.length > 0, { timeout: 8000 });
    const captured = await page.evaluate(() => window.__copiedCareDates);
    assert.match(captured, /CLEARDOC NEXT DATES/, "the next-dates digest must carry a header");
    assert.match(captured, /\(in |\(today|\(tomorrow|overdue/, "the next-dates digest must include countdowns");
    assert.match(captured, /_Generated by ClearDoc — informational only, not legal advice_/, "the next-dates digest must carry the disclaimer");
    assert.equal(errors.length, 0, `zero console errors, got: ${errors.join(" | ")}`);
    // Cycle #282 v2 — 't' shortcut copies the same digest.
    await page.evaluate(() => { window.__copiedCareDates = null; });
...
skip("analyzer: care plan copies as Markdown", async () => {
  ...
  try {
    ...
    assert.match(captured, /http.*#share=/, "the copied Markdown must include a share link");
...
skip("analyzer: care plan deadline digest copies dated items", async () => {
  ...
  const html = require("node:fs").readFileSync(require("node:path").join(ROOT, "analyze.html"), "utf8");
  const appSrc = require("node:fs").readFileSync(require("node:path").join(ROOT, "assets", "app.js"), "utf8");
  assert.match(html, /id="careDeadlineDigestBtn"/, "analyze.html must expose the care plan deadline digest button");
  assert.match(html, /title="Copy just the deadlines with countdowns"/, "the button must be labelled as a deadline digest");
  ...
  page.on("pageerror", (e) => errors.push(String(e)));
  await page.addInitScript(() => {
    window.__copiedDeadlineDigest = null;
    try {
      Object.defineProperty(navigator, "clipboard", {
        configurable: true,
        value: {
          writeText: async (txt) => { window.__copiedDeadlineDigest = txt; },
          write: async () => {},
        },
      });
    } catch (_) {
      try { navigator.clipboard = { writeText: async (txt) => { window.__copiedDeadlineDigest = txt; }, write: async () => {} }; } catch (_2) {}
    }
  });
  try {
    await page.goto(`http://127.0.0.1:${PORT}/analyze.html`, { waitUntil: "networkidle" });
    await page.click(".qf[data-fill]:first-of-type");
    await page.click("#analyzeBtn");
    await page.waitForSelector("#careBlock:not([hidden]) #careDeadlineDigestBtn", { timeout: 8000 });
    await page.click("#careDeadlineDigestBtn");
    await page.waitForFunction(() => window.__copiedDeadlineDigest && window.__copiedDeadlineDigest.length > 0, { timeout: 8000 });
    const captured = await page.evaluate(() => window.__copiedDeadlineDigest);
    assert.match(captured, /CLEARDOC DEADLINE DIGEST —/, "the digest must carry a header");
    assert.match(captured, /\(in |\(today|\(tomorrow|overdue/, "the digest must include countdowns");
    assert.match(captured, /http.*#share=/, "the digest must include a share link");
    assert.match(captured, /_Generated by ClearDoc — informational only, not legal advice_/, "the digest must carry the disclaimer");
    assert.equal(errors.length, 0, `zero console errors, got: ${errors.join(" | ")}`);
    // Cycle #291 v2 — 'e' shortcut copies the same digest.
    await page.evaluate(() => { window.__copiedDeadlineDigest = null; });
    await page.keyboard.press("e");
    await page.waitForFunction(() => window.__copiedDeadlineDigest && window.__copiedDeadlineDigest.length > 0, { timeout: 8000 });
    const viaKey = await page.evaluate(() => window.__copiedDeadlineDigest);
    assert.match(viaKey, /CLEARDOC DEADLINE DIGEST —/, "the e shortcut must copy the deadline digest");
  } finally {
    await page.close();
    await ctx.close();
  }
...
skip("analyzer: copy-all bundle combines key facts, digests, and next steps", async () => {
  ...
  const html = require("node:fs").readFileSync(require("node:path").join(ROOT, "analyze.html"), "utf8");
  const appSrc = require("node:fs").readFileSync(require("node:path").join(ROOT, "assets", "app.js"), "utf8");
  assert.match(html, /id="copyBundleBtn"/, "analyze.html must expose the copy-all bundle button");
  ...
  page.on("pageerror", (e) => errors.push(String(e)));
  await page.addInitScript(() => {
    window.__copiedBundle = null;
    try {
      Object.defineProperty(navigator, "clipboard", {
        configurable: true,
        value: {
          writeText: async (txt) => { window.__copiedBundle = txt; },
          write: async () => {},
        },
      });
    } catch (_) {
      try { navigator.clipboard = { writeText: async (txt) => { window.__copiedBundle = txt; }, write: async () => {} }; } catch (_2) {}
    }
  });
  try {
    await page.goto(`http://127.0.0.1:${PORT}/analyze.html`, { waitUntil: "networkidle" });
    await page.click(".qf[data-fill]:first-of-type");
    await page.click("#analyzeBtn");
    await page.waitForSelector("#copyBundleBtn", { timeout: 8000 });
    await page.click("#copyBundleBtn");
    await page.waitForFunction(() => window.__copiedBundle && window.__copiedBundle.length > 0, { timeout: 8000 });
    const captured = await page.evaluate(() => window.__copiedBundle);
    assert.match(captured, /CLEARDOC ANALYSIS BUNDLE/, "bundle must carry a clear header");
    assert.match(captured, /http.*#share=/, "bundle must include a share link");
    assert.match(captured, /_Generated by ClearDoc — informational only, not legal advice\._/, "bundle must carry the disclaimer");
    assert.equal(errors.length, 0, `zero console errors, got: ${errors.join(" | ")}`);
  } finally {
    await page.close();
    await ctx.close();
...
skip("analyzer: risk email button opens a pre-filled mail client", async () => {
  ...
  try {
    ...
    assert.equal(errors.length, 0, `zero console errors, got: ${errors.join(" | ")}`);
    // Cycle #296 v2 — 'i' shortcut opens the same email.
    await page.evaluate(() => { window.__riskMailto = null; });
    await page.keyboard.press("i");
    await page.waitForFunction(() => window.__riskMailto && window.__riskMailto.startsWith("mailto:"), { timeout: 8000 });
    const viaKey = await page.evaluate(() => window.__riskMailto);
    assert.match(viaKey, /^mailto:\?subject=/, "the i shortcut must open the risk email");
  } finally {
    await page.close();
    await ctx.close();
  }
...
skip("analyzer: deadline digest copies urgency-grouped deadlines", async () => {
  ...
  page.on("pageerror", (e) => errors.push(String(e)));
  await page.addInitScript(() => {
    window.__copiedDeadlines = null;
    try {
      Object.defineProperty(navigator, "clipboard", {
        configurable: true,
        value: {
          writeText: async (txt) => { window.__copiedDeadlines = txt; },
          write: async () => {},
        },
      });
    } catch (_) {
      try { navigator.clipboard = { writeText: async (txt) => { window.__copiedDeadlines = txt; }, write: async () => {} }; } catch (_2) {}
    }
  });
  try {
    await page.goto(`http://127.0.0.1:${PORT}/analyze.html`, { waitUntil: "networkidle" });
    await page.click(".qf[data-fill]:first-of-type");
    await page.click("#analyzeBtn");
    await page.waitForSelector("#deadlineDigestBtn", { timeout: 8000 });
    await page.click("#deadlineDigestBtn");
