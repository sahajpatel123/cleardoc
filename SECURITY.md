...
- **Hardening script gates**: `scripts/security-hardening.sh` fails if CSP `report-uri`, `frame-ancestors`, `object-src`, or `base-uri` regress, or if production HTML reintroduces inline event handlers; it also verifies workflow actions are SHA-pinned, workflow permissions default to read-only, every workflow job declares a timeout, workflows declare concurrency groups, `security.txt` has the RFC 9116 `Contact` field, and `SECURITY.md` exposes a reporting channel.
