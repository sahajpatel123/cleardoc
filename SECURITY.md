...
- **Hardening script gates**: `scripts/security-hardening.sh` fails if CSP `report-uri`, `frame-ancestors`, `object-src`, `base-uri`, `default-src`, or `upgrade-insecure-requests` regress, if `connect-src`/`script-src` gain wildcards, if page/API security headers loosen, if high-severity npm audit gates, `npm ci`, npm cache, or JSON validation disappear from CI, if browser smoke tests disappear, if test servers regress to binding all interfaces (they must stay on `127.0.0.1`), if hardcoded secret scanning weakens (it must scan tracked files only and detect client secrets, provider tokens, and private keys), if workflows download-and-execute remote scripts (no `curl|sh` / `curl|bash`), if package.json adds `preinstall` / `install` / `postinstall` lifecycle scripts, if CodeQL re-enables fail-fast, loses `security-events`, or drops its test-path ignore, if checkout persists credentials, if workflows lose `workflow_dispatch`, if the hardening script stops running in CI, if dependabot loses an ecosystem, if CODEOWNERS loses owner assignments, if `security.txt` `Expires` lapses, loses its rewrite, canonical/language directives, or policy URL, if `package-lock.json` is untracked, if `robots.txt` loses its `/api/` block, `User-agent: *`, `Allow: /`, standard directives, or canonical sitemap URL, if `sitemap.xml` loses core URLs, lastmod entries, changefreq entries, or priority entries, if `vercel.json` loses clean URLs or gains trailing slashes, or if production HTML reintroduces inline event handlers or inline scripts; it also verifies workflow actions are SHA-pinned, workflow permissions default to read-only, every workflow job declares a timeout, workflows declare concurrency groups, `security.txt` has the RFC 9116 `Contact` field, and `SECURITY.md` exposes a reporting channel, the private vulnerability reporting URL, and the security email.
