...
- **Risk summary footer** (iter #200) — under the risk radar, a one-line tally now reads `Found N risks · X trap · Y watch · Z note` so users see the severity distribution at a glance without counting cards. Computed locally from `lastFlags`, hidden when there are zero flags so it never renders in the pre-analysis empty state. Reuses the existing `metaline` style for visual consistency with the rewrite stats footer.
