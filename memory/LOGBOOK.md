...
## Cycle #377 — 2026-08-23 — ADD
**Changes Made:** New exit-rights lens (🚪 "Who can end this") — sibling of liability symmetry. detectTermRights scans termination sentences for convenience exits ("for convenience" / "without cause" / "for any reason" / "at any time" / "sole discretion"), attributes the holder via capitalized word-shape partyOf (may/shall-be-entitled/reserves-the-right + terminate/cancel), and reads the notice window (digits or word-forms: "30 days'" / "sixty days'"). Exactly one named holder and no either-party language → finding "Only 'Company' can walk away without cause" with the notice window folded into the why and a jump span. Mutual exits and cause-only exits stay quiet (absence of any termination clause stays with the what-if surface). Full house equipment: termBlock/termNote/termList refs + analyze.html block + guarded call site + cheat-sheet section "Exit rights (who can end this)" + ask-list row "Make the exit mutual" + landing chip #11 "Exit rights". Scope-checks this window killed the two carried candidates: indemnity direction already attributed in obligation-balance (push you/them/shared, ~13438) and auto-renewal fully covered (RENEW_RE + window math ~10896).
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 323 pass / 0 fail / 141 skipped · integration 16/16.
**Prompt Intention:** Loop discipline held (polish→add). Next cycle #378 is POLISH of the exit-rights lens — candidates: cure-period asymmetry (cause exits without a cure window), notice-window sanity (suspiciously short windows like 0–5 days), or exit + notice cross-reference.

## Cycle #378 — 2026-08-23 — POLISH
**Changes Made:** Exit-rights findings now grade the window instead of quoting it neutrally. detectTermRights parses the captured notice window into a day count (digit prefix or local DAYS_WORD map — self-contained so the test extraction stays pure) and sharpens the one-way why: ≤7 days → "that is almost no warning; you can be dropped mid-job"; no window stated at all → "with no notice period stated at all". Normal windows (e.g. 30/60 days) keep the plain phrasing. Behavioral tests gained both tiers: shortDoc ("5 days'" → /almost no warning/) and nowinDoc ("at any time and for any reason" → /no notice period stated/), plus a structural pin.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 323 pass / 0 fail / 141 skipped · integration 16/16. One iteration: the structural pin's wording didn't match the shipped comment text; aligned the pin to the real marker (behavioral corpora passed first try).
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #379 is ADD — scope-check incumbents first. Remaining polish candidates parked here: cure-period asymmetry in cause-only exits; exit + notice cross-reference.

## Cycle #379 — 2026-08-23 — ADD
**Changes Made:** New breach-alerting lens (🔔 "If your data leaks") — how fast must a leak reach you? detectDataBreaches gates on data-handling language (personal information / PII / confidential information / privacy) so non-data docs stay quiet, then grades every breach-notice sentence against the 72-hour norm: compliant clocks (72h or less, digits or word-forms via NUM_WORDS incl. compounds like "seventy-two", day units ×24) stay quiet; slow clocks quote their raw window ("within ten business days"); weasel clocks ("promptly" / "without undue delay" / "as soon as practicable") are named as uncountable; breach talk with no clock at all says so. Data-handling docs silent on alerting (>500 chars) get the missing-provision row with no span. Full house equipment: breachBlock refs + analyze.html block + guarded call site + cheat-sheet "Breach alerting (if data leaks)" + ask-list "Pin the breach clock" + landing chip #12 "Breach notice". Scope-checks killed IP-direction (RISK rule 4623 owns transfer traps) and confidentiality survival (perpetual-trap rule 4596 owns it).
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 324 pass / 0 fail / 141 skipped · integration 16/16. Two honest iterations: (1) self-caught under-padded silentDoc corpus (<500 chars would have passed for the wrong reason — same trap as #374); (2) first full-gate run showed integration 15/16 with compare-panel download timing out on #compareStats table — passed twice on re-run (6.9s), diff touches no compare path; ruled a load flake from back-to-back suite pressure.
**Prompt Intention:** Loop discipline held (polish→add). Next cycle #380 is POLISH of the breach lens — candidates: clock-vs-severity pairing (clock attached to which party's duty), "without undue delay AND within X hours" dual forms, or parked exit-lens items (cure-period asymmetry).

## Cycle #380 — 2026-08-23 — POLISH
**Changes Made:** Breach-clock parser now tolerates contract house style: parenthesized numeric duplicates ("seventy-two (72) hours", "thirty (30) days") parse instead of falling through to the no-clock verdict. Dual-form semantics pinned by test — "notify without undue delay, AND in any event within seventy-two (72) hours" stays quiet because the hard bound governs; the paren form also verified on a slow clock ("within thirty (30) days" fires, normalized raw quoted). Structural pin added.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 324 pass / 0 fail / 141 skipped · integration 16/16 (first-try green, no flakes this cycle).
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #381 is ADD — scope-check incumbents first. Parked candidates: breach clock attribution (who must tell whom), exit-lens cure-period asymmetry, payment net-term extraction.

## Cycle #381 — 2026-08-23 — ADD
**Changes Made:** New money-timing lens (💰 "Money timing") — whose cash floats whose business. detectPayFlow collects one "flow" per paying party via capitalized word-shape partyOf (X shall/must/will pay|remit|refund|reimburse|return), mutual sentences skipped; each flow keeps the longest within-N-days clock seen and ORs interest flags ("without interest" / "interest-free" clear it). Two verdicts: (1) ≥2 clocked payers ≥30 days apart → "Money moves on two very different clocks" with both windows quoted, span on the slower sentence; (2) exactly one of ≥2 flows carries interest → "Being late only costs 'X'" naming the free rider. Matched clocks, single-clock docs, and mutual duties stay quiet. Full house equipment: payBlock refs + analyze.html block + guarded call site + cheat-sheet "Money timing (who waits to be paid)" + ask-list "Even out the money clocks" + landing chip #13.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 325 pass / 0 fail / 141 skipped · integration 16/16 — first full-gate run showed 14/16 with two 8s waitForSelector timeouts (signing-checklist CSV, decision block) while the preceding smoke run took 43s under load; both passed in the standalone re-run. Same flake family as #379's compare timeout — third occurrence of the signature.
**Prompt Intention:** Loop discipline held (polish→add). Next cycle #382 is POLISH — leading candidate is now infrastructure: harden the integration suite against back-to-back load flakiness (raise the 8s waitForSelector budget or serialize the gate), so gate runs stop needing manual re-verification.

## Cycle #382 — 2026-08-23 — POLISH
**Changes Made:** Integration harness hardened against the recurring back-to-back load flake (compare table #379; action grid + decision block #381). All 21 positive content-visibility waits in test/integration.test.js now share a SEL_WAIT = 20000 budget (was hardcoded 8000ms) with a comment explaining why: under suite-on-suite load the wait, not the page, was the bug. Deliberately untouched: the tight 2–4s budgets that watch state changes after content is already visible (.reading-row-done, .act-checked) and negative assertions — there a slow wait would hide real regressions.
**Gate:** node --check ✓ · unit 490/490 · smoke 325 pass / 0 fail / 141 skipped · integration 16/16 — twice consecutively, back-to-back full gates, the exact condition that flaked before. No manual re-runs needed.
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #383 is ADD — scope-check incumbents first. Parked candidates: breach clock attribution, exit-lens cure-period asymmetry, payment-amount extraction to pair with money-timing clocks.

## Cycle #383 — 2026-08-23 — ADD
**Changes Made:** New transfer-rights lens (🔁 "Who can hand this off") — who may assign the deal onward. detectXfer scans assignment/delegation sentences; a "free" handoff is either explicit ("may freely assign … without consent") or an unqualified "may assign" with no consent/approval fence within 120 chars after the modal; partyOf (word-shape, may/have-the-right + assign) attributes it; mutual/either/neither-party clauses and consent-fenced assignments never count as free. Verdict: exactly one free party → "Only 'Company' can hand off this contract", noting when the other side is explicitly fenced ("shall not assign"), with the buyer/affiliate risk spelled out and asks ready (either-party-may-assign / consent-not-unreasonably-withheld). Full house equipment: xferBlock refs + analyze.html block + guarded call site + cheat-sheet "Transfer rights (who can hand off the deal)" + ask-list "Even out transfer rights" + landing chip #14 "Transfers". Scope-check: assignment direction was unclaimed — only generic traps (IP-assignment 4625, coverage 9467, RISK_TOKEN) existed.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 326 pass / 0 fail / 141 skipped · integration 16/16 — full gate green on the first try, confirming the #382 harness hardening holds under back-to-back load.
**Prompt Intention:** Loop discipline held (polish→add). Next cycle #384 is POLISH of the transfer lens — candidates: merger/change-of-control carve-outs surfaced in the why ("except to a merger acquirer"), notice requirement extraction from the free sentence, or parked lens items.

## Cycle #384 — 2026-08-23 — POLISH
**Changes Made:** Transfer lens now grades carve-out scope and notice-only handoffs. "May assign to an affiliate / in connection with a merger / to a successor" (SCOPE_RE) is no longer treated as a door open to anyone: scoped rights fire only when the other side is explicitly fenced, under their own headline "Their hand-off rights outrank yours" with the carve-out spelled out and a mirror-the-carve-outs ask; unscoped free rights keep the original headline. Notice-only handoffs ("upon notice to", "notice to") surface as "It asks only for advance notice — your signature is not needed." Tests: scoped+fence fires new headline; scoped-without-fence stays quiet (no-noise rule pinned); plain unscoped corpus rebuilt article-free after partyOf grabbed "The Provider" (same corpus discipline as #367).
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 326 pass / 0 fail / 141 skipped · integration 16/16. One iteration: the article-led corpus mis-capture, self-caught in the first run.
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #385 is ADD — scope-check incumbents first. Parked candidates: breach clock attribution, exit-lens cure-period asymmetry, insurance-requirement lens.

## Cycle #385 — 2026-08-23 — ADD
**Changes Made:** New insurance lens (🩺 "Insurance duties") — a coverage promise without a number is a promise to argue later. detectInsurance gates on duty verbs near "insurance" (maintain/carry/procure/obtain/keep-in-force), counts duties, and flags the first one with no stated limit: STATED_RE accepts a dollar figure, "not less than", "limits of", "per occurrence/claim", or "combined single limit" — anything countable beats "adequate". Finding: "An insurance duty with no number attached" with a concrete ask ($1,000,000 per occurrence + certificates on request) and a jump span. Mutual coverage duties and insurance-free documents stay quiet. Full house equipment: insBlock refs + analyze.html block + guarded call site + cheat-sheet "Insurance duties (who carries the coverage)" + ask-list "Pin down the insurance numbers" + landing chip #15 "Insurance". Also carried through: a local repair found in the working tree (orphaned Iter #159 assertions moved back inside their test) — user's edit, verified by the gate, committed alongside.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 327 pass / 0 fail / 141 skipped · integration 16/16 — first-try green.
**Prompt Intention:** Loop discipline held (polish→add). Next cycle #386 is POLISH of the insurance lens — candidates: surface the stated limit in the count line ("duties found · limits stated: N"), certificate-delivery requirement detection, or parked lens items.

## Cycle #386 — 2026-08-23 — POLISH
**Changes Made:** Insurance lens now names its party and tallies its numbers, matching the house voice of every sibling lens. detectInsurance attributes the unnumbered duty via word-shape partyOf (X shall/must/will maintain|carry|procure|obtain) → headline becomes "'Contractor' owes coverage with no number attached" (generic fallback kept when attribution fails); return shape gains `stated`, and the count line reads "N insurance sentences examined · M duties found · K with stated limits" so a mixed document shows both the numbered duty and the hole side by side. Tests: attributed-label assert, mixed corpus pinning duties=2/stated=1/items=1, structural pins for the tally string and the attribution marker.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 327 pass / 0 fail / 141 skipped · integration 16/16 — first-try green.
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #387 is ADD — scope-check incumbents first. Parked candidates: breach clock attribution, exit-lens cure-period asymmetry, certificate-of-insurance delivery detection.

## Cycle #387 — 2026-08-23 — ADD
**Changes Made:** New publicity lens (📣 "Your name in their ads") — whether the contract hands out marketing rights over your name, logo, and work. detectPublicity gates on a real grant (GRANT_A verb→asset within 80 chars: use/display/publish/reproduce/identify/refer-to/list … name/logo/trademark/likeness/work product/deliverables; GRANT_B catches reverse order like "deliverables may be reproduced"), fences on approval language (consent/approval/sign-off nearby → quiet), and grades severity via PERPETUAL_RE (perpetual/irrevocable/survives termination). partyOf attributes the grantee ("'Client' can put your name on its marketing"; generic fallback when word-shape fails); perpetual grants add "— and the right never expires with the contract". Mutual grants and publicity-free documents stay quiet. Full house equipment: pubBlock refs + analyze.html block 📣 + guarded call site (last in render order) + cheat-sheet "Publicity rights (your name in their ads)" + ask-list "Put approvals around publicity" + landing chip #16 "Publicity". Scope-check before building: setoff was the only other unclaimed surface; publicity chosen for user impact.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 328 pass / 0 fail / 141 skipped · integration 15/16 in-gate → decision-block waitForSelector blew even the 20s SEL_WAIT under back-to-back load; standalone re-run passed at 6.7s — fourth occurrence of the load-flake family, first to exceed the hardened budget (noted for a future harness cycle). Three test iterations self-caught: partyOf missing whitespace before "use", over-specified label assert, reverse-order grant corpus.
**Prompt Intention:** Loop discipline held (polish→add). Also carried through with attribution: the user's in-flight .github/workflows/security.yml edit (eval-gate excludes test/, fixing the already-red Security checks job caused by smoke tests' new Function(src) introspection) — verified locally that old gate flags 21 test lines and the fix clears them on CI's GNU grep; noted that macOS BSD grep strips ./ so the filter is a no-op locally. Next cycle #388 is POLISH of the publicity lens — candidates: scope-word enumeration ("name AND logo" shown as multi-asset), exclusions surfaced ("except in connection with…"), or parked lens items.

## Cycle #388 — 2026-08-23 — POLISH
**Changes Made:** Publicity lens now enumerates the whole grant instead of reporting only the first asset the eye lands on. detectPublicity collects every distinct asset (name/logo/trademark/mark/likeness/work product/deliverables, plural forms canonicalized) across ALL live grant sentences — mutual and approval-fenced sentences excluded from the collection too — capped at four so headlines stay readable, natural-joined ("name and logo" / "name, logo, likeness, and work product"). Headline "'Client' can put your name, logo, and likeness on its marketing"; generic fallback unchanged. One self-caught bug during implementation: the redundant gm[1] re-push could add a non-canonical duplicate ("logo and logos") after ASSET_ALL canonicalization — dropped. Tests: single-sentence triple enumeration, cross-sentence accumulation, duplicate collapse, cap-with-tail-dropped, structural pin.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 328 pass / 0 fail / 141 skipped · integration 15/16 in-gate with "reading list speaks…" timing out at the 20s budget — a DIFFERENT test than #387's victim (decision block), confirming the rotation signature of load flake; standalone re-run passed at 5.8s.
**Prompt Intention:** Loop discipline held (add→polish). Mid-cycle the user raised the bar explicitly: commit and push as much and as often as possible, no limits — so this wakeup ships a second change as its own commit: raising the integration SEL_WAIT budget that has now needed manual re-runs two cycles running. Next cycle #389 is ADD.

**#388 follow-up (same wakeup, separate commit per the user's raised bar):** SEL_WAIT 20000 → 30000 in test/integration.test.js — the 20s budget itself flaked twice running (#387 decision block, #388 reading list) with observed in-gate waits near 23s, a different test losing the lottery each run. Same positive-content semantics, headroom over the observed worst case. Gate: integration 16/16 standalone on the new budget.

## Cycle #389 — 2026-08-23 — ADD
**Changes Made:** New setoffs lens (✂️ "Deductions & setoffs") — who can snip money out of your invoice. detectSetoff gates on set-off/offset/deduct language (plural "deductions" fixed after first test run), attributes one-way offset rights via word-shape partyOf ("Company may offset…" → "'Company' can deduct from what it owes you"), catches passive-voice short-pay drafting ("payments may be reduced" — REDUCED_RE fallback after the anonymous corpus exposed the gap), and never fires on mutual/reciprocal clauses. Ask baked into the why: offsets mutual and capped at amounts actually disputed. Full house equipment: setoffBlock refs + analyze.html block ✂️ + guarded call site (after publicity) + cheat-sheet "Setoffs (who can short-pay whom)" + ask-list "Cap the setoffs" + landing chip #17.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 330 pass / 0 fail / 141 skipped · integration 16/16 — full gate green. Three self-caught bugs before ship: plural-form regex gap, missing passive fallback (0 !== 1), null-party crash on the passive path.
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #390 is POLISH of the setoffs lens — candidates: cap/carve-out grading ("except for amounts disputed in good faith"), attribution of the benefiting party, or parked lens items.

## Cycle #390 — 2026-08-23 — POLISH
**Changes Made:** Setoffs lens grades carve-out scope (mirrors the transfer lens's #384 lesson): an offset limited to "amounts actually disputed" / "in good faith" / a bounded sum (CAP_NEAR) no longer draws the unlimited-deduct alarm — it earns "deduct — but only what is actually disputed", a why crediting the guardrail while noting it still runs one way, and the mutual-rights ask. Uncapped one-way offsets keep full strength. One dead conditional self-caught and removed from the draft. Test: capped corpus fires graded headline + guardrail credit; structural pin.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 330 pass / 0 fail / 141 skipped · integration 16/16.
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #391 is ADD — scope-check incumbents first (amendment/venue/jury all claimed on check).

## Cycle #391 — 2026-08-23 — ADD
**Changes Made:** New rate-changes lens (📈 "Rate changes") — who can raise your price after you sign. Scope-check: amendment (3 incumbents), venue/jurisdiction (jurisBlock), jury/class-action (traps+glossary), force majeure, non-solicit all claimed; price-increase rights had ZERO coverage → built. detectRates catches both drafting directions (verb→fee HIKE_A; passive "fees shall be adjusted" HIKE_B), attributes via partyOf ("Company may increase…" → "'Company' can raise your rates at any time"), grades caps/indexes (CAP_NEAR: not-exceed/capped/%/CPI/inflation) into "within stated bounds" with the guardrail credited, and asks for a cap + advance notice + penalty-free exit window. Passive fallback fires on whatever HIKE_B matched (first iteration demanded "may be increased" and missed "shall be adjusted" — caught by own tests). Full house equipment: rateBlock refs + analyze.html block 📈 + guarded call site + cheat-sheet "Rate changes (who can raise the price)" + ask-list "Cap the rate hikes" + landing chip #18.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 331 pass / 0 fail / 141 skipped · integration 16/16 — full gate green. NOTE: pushes for #388-followup/#389/#390/#391 were blocked behind repeated tool-gate outages; all four changesets verified green before this entry and flushed together immediately after.
**Prompt Intention:** Loop discipline held (add→add only because #390's polish was completed in its own wakeup before this build began). Next cycle #392 is POLISH of the rate lens — candidates: notice-window extraction ("upon thirty days notice" surfaced like breach clocks), most-favored-nation pricing asks, or parked items.

## Cycle #392 — 2026-08-23 — POLISH
**Changes Made:** Rate lens surfaces the notice window, mirroring how breach clocks were graded in #378. NOTICE_WIN captures "upon/after/with N days notice" (digits or word-numbers, paren tolerance); a quoted window lands in the why as "It does promise thirty days' notice — time to renegotiate, or leave, before the new price lands", and an uncapped hike with no window now says outright "No notice period is stated at all". Capped hikes stay quiet about notice unless one exists. Tests: hike corpus asserts the quoted window; new ambush corpus (uncapped, no notice) pins the blunt line; structural pin.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 331 pass / 0 fail / 141 skipped · integration 16/16 — full gate green.
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #393 is ADD — scope-check incumbents first; remaining unclaimed surfaces are thinning, so candidates include splitting the traps/risk-map coverage into dedicated lenses where a single regex currently carries a whole topic.

## Cycle #393 — 2026-08-23 — ADD
**Changes Made:** New indemnity lens (🧯 "Indemnity") — who covers whose losses. Scope-check justified the build: the entire topic rode on ONE generic trap regex (indemnif|hold harmless, no direction, no attribution, no scope) plus plain-language substitutions. detectIndemnity attributes one-way coverage promises via word-shape partyOf ("Consultant shall defend and indemnify…" → "'Consultant' covers the other side's losses"), grades breadth via BROAD_RE ("any and all" / "howsoever caused" / legal-fees-inclusive → the why spells out paying before fault is decided), stays quiet on mutual/reciprocal indemnities, and asks for mutual-capped-excluding-gross-negligence. Full house equipment: indemBlock refs + analyze.html block 🧯 + guarded call site + cheat-sheet "Indemnification (who covers whose losses)" + ask-list "Make indemnity mutual" + landing chip #19 "Indemnity". data-im/_imWired prefix chosen to avoid colliding with id-like attribute names.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 332 pass / 0 fail / 141 skipped · integration 16/16 — full gate green, first-try on the new lens.
**Prompt Intention:** Loop discipline held (polish→add). Next cycle #394 is POLISH of the indemnity lens — candidates: carve-out grading (gross-negligence exclusions credited), defense-vs-payment obligation split, notice-of-claim requirement surfaced.

## Cycle #394 — 2026-08-23 — POLISH
**Changes Made:** Indemnity lens credits fault-bounded duties (FAULT_NEAR: "to the extent", "in proportion to", "solely from… negligence"). A one-way indemnity bounded to losses the payer actually causes now earns "This one is bounded to losses that party actually causes — the fairer shape." in its why; broad "any and all" scopes are structurally excluded from the credit so the alarm keeps full strength. Tests: narrow corpus asserts the credit line lands AND the broad corpus is denied it; structural pin.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 332 pass / 0 fail / 141 skipped · integration 15/16 in-gate ("templates can be duplicated" blew the 30s budget at 33.7s — sixth rotation victim) → standalone re-run passed at 6.3s.
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #395 is ADD — scope-check incumbents first; candidates thinning further: warranty-disclaimer lens ("as-is" coverage), confidentiality-scope lens, or splitting other single-regex trap topics per the #392 note.

## Cycle #395 — 2026-08-23 — ADD
**Changes Made:** New non-compete lens (🚧 "Non-compete") — where you can't work next. Scope-check: the topic rode on one DECISION_PATTERNS scoring token plus a RISK_TOKEN mention, nothing analytical. detectNoncompete gates on both hyphenated ("non-compete") and verb-form ("shall not compete/solicit/accept employment") drafting — the first gate missed verb forms entirely (checked=0, caught by own tests); attributes the bound party via "X shall/covenants to not" word-shape; extracts duration with word-number support (three→thirty-six, paren tolerance) after the light corpus exposed missing small numbers ("six months"); grades three tiers: >12mo draws "shrink it to a year or less", ≤12mo asks only scope-narrowing to direct competitors, and NO stated duration becomes the headline itself ("may not compete with no end date stated at all"). GEO_RE notes worldwide/unbounded territory. Full house equipment: ncBlock refs + analyze.html block 🚧 + guarded call site + cheat-sheet "Restrictive covenants (where you can't work next)" + ask-list "Shrink the non-compete" + landing chip #20.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 333 pass / 0 fail / 141 skipped · integration 16/16 — full gate green. Two self-caught bugs before ship: verb-form gate gap, small word-number duration gap.
**Prompt Intention:** Loop discipline held (add→polish). Next cycle #396 is POLISH of the non-compete lens — candidates: garden-leave/paid-restriction carve-outs credited, carve-out grading (fiduciary/insider exceptions), or parked items.

## Cycle #396 — 2026-08-23 — POLISH
**Changes Made:** Non-compete lens grades two signals it previously ignored. (1) Garden-leave credit: a second scan over covenant-context segments flags "garden leave" / "continue to receive … salary" drafting and the finding's why earns " This one keeps paying while it restrains — garden leave, the fairer shape." before the ask; unpaid covenants earn nothing (asserted negatively). (2) Blue-pencil named as its own finding: "reduced/reformed to the maximum (extent permissible)" / "to the extent such restriction" clauses become a second jump-row — "Even if struck down, this covenant rewrites itself" — with why reading the auto-substitution as intent ("the drafter knows the reach is excessive and wants the harshest version enforced anyway"). RESTRICT_CTX gate keeps liability-cap "maximum extent permitted by law" from ever triggering the warning (false-positive guard test added). Structural pin: "// Cycle #396 — polish: pay while restrained; self-rewriting covenants."
**Gate:** node --check ✓ · unit 490/490 · smoke 333 pass / 0 fail · integration 15/16 in-suite — CSV-tracker download blew its wait budget (35.9s) under suite load, the documented rotation-flake family's fourth victim (decision #387, reading list #388, templates #394) — passed standalone 3/3 in seconds immediately after. No regression.
**Prompt Intention:** Loop discipline held (polish of #395's lens). Next cycle #397 is ADD — scope-check candidates before building: warranty-disclaimer and confidentiality surfaces (both previously judged token-covered), or splitting another single-regex trap topic into a dedicated lens.

## Cycle #397 — 2026-08-23 — ADD
**Changes Made:** New disclaimers lens (🏷️ "Disclaimers") — what is NOT promised, and whether the waiver even holds. Scope-check: warranty rode only on a risk-map hint and tokens (auto-renewal deeply served; confidentiality carries real analysis) — thinnest surface left. detectWarranty gates on disclaim/as-is/where-is/all-faults/no-warranty/waive-warrant drafting with a lookahead keeping "as is customary" false friends quiet; attributes the drafter via staged mixed-case name matching ("AS IS" can never pose as a party) + case-insensitive verb verification; grades conspicuousness (caps "AS IS" → "drafted to be seen, court will likely hold every word"; buried lowercase → "courts have refused waivers that were not conspicuous"); names UCC implied-waiver clauses (merchantability / fitness for particular purpose) as their own finding; express-promise presence flips the ask from demanding a new 90-day workmanship warranty to holding them to their existing list. Two self-caught bugs before ship: leftmost-match captured "AS IS and Company" as the party; de-pluralizing /i made verbs case-sensitive. Full house equipment: warBlock refs + analyze.html block 🏷️ + guarded call site + cheat-sheet "Disclaimers (what is NOT promised)" + ask-list "Add a 90-day warranty" + landing chip #21.
**Gate:** node --check ✓ · unit 490/490 · smoke 334 pass / 0 fail / 140 skipped · integration 16/16 in-suite, no flake.
**Prompt Intention:** Loop discipline held (add after #396 polish). Next cycle #398 is POLISH of the disclaimers lens — candidates: reverse-direction grading (all-inclusive warranties that overpromise), conspicuousness for implied-waiver findings too, or parked items.

## Cycle #398 — 2026-08-23 — POLISH
**Changes Made:** Disclaimers lens gains three graders. (1) Absolute overpromises named as their own finding: warrant/guarantee/promise gate + error-free/uninterrupted/completely-secure/100% drafting → "An absolute promise nobody can keep" — why calls the two-sided play ("this promise made the sale, and the disclaimer next door avoids paying on it") and asks for a measurable standard (99.9% uptime target with service credit). Absolute promises count as express promises for the ask-flip. (2) Express promises get dated: EXP_DUR_RE lifts "ninety days"-style shelf lives from express/absolute segments and the blanket ask quotes it back — "those promises expire in ninety days — hold them to that list." (3) Conspicuousness extended to implied-waiver findings: all-caps implied waivers read as deliberate ("argue it before signing, not after"), quiet lowercase ones earn the burial argument. Structural pin: "// Cycle #398 — polish: overpromises graded; waivers dated and weighed."
**Gate:** node --check ✓ · unit 490/490 · smoke 334 pass / 0 fail / 140 skipped · integration 16/16 in-suite, no flake. Standalone disclaimers block green first run.
**Prompt Intention:** Loop discipline held (polish of #397's lens). Next cycle #399 is ADD — candidates from parked list: confidentiality survival/perpetuity grading as a full lens is partly served; stronger candidate may be splitting another single-regex trap topic or a non-lens feature (workspace polish). Scope-check before building.

## Cycle #399 — 2026-08-23 — ADD
**Changes Made:** New confidentiality lens (🔒 "Confidentiality") — who owes whose secrets. Scope-check: payment timing already served by detectPayFlow; confidentiality carried only a trap token, risk-map counter, and mutual-NDA push — no direction/duration/breadth analysis. detectConfid attributes the bound party via two shapes ("shall keep … confidential" / "shall not disclose") with case-sensitive staged name matching; grades three dials: direction (mutual drafting earns the both-ways fairness credit even unattributed, via firstConfidAt anchor fallback), duration (bounded survival terms quoted back — "outlives signing by three years — the fairer shape"; document-wide PERP_RE scan because survival sentences stop saying "confidential" mid-draft → "The silence never expires" finding with the 3-5-year ceiling ask), breadth (disclosure-context lookaheads keep "orally/in any form/whether or not marked" honest → "Everything spoken becomes a secret"). Self-caught bug before ship: trailing /i on partyOf let lowercase "party" pose as a bound party (third /i-stripping fix this week — pattern now documented in-code). Full house equipment: cfBlock refs + analyze.html block 🔒 + guarded call site + cheat-sheet "Confidentiality (who owes whose secrets)" + ask-list "Cap the secrecy" + landing chip #22.
**Gate:** node --check ✓ · unit 490/490 · smoke 335 pass / 0 fail / 139 skipped · integration 16/16 in-suite, no flake.
**Prompt Intention:** Loop discipline held (add after #398 polish). Lens family now 22 surfaces. Next cycle #400 is POLISH of the confidentiality lens — candidates: return-or-destroy-of-materials on exit, residual-knowledge clauses, definition-of-confidential-info carve-outs (public domain, independently developed).

## Cycle #400 — 2026-08-23 — POLISH
**Changes Made:** Confidentiality lens grades exits, memories, and carve-outs — the 400th logged completion. (1) Residual-knowledge clauses named as their own finding: document-wide RESID_RE (unaided memory / retained-in-memory / residual+knowledge pairing, so leasing "residual value" stays quiet) → "Whatever you remember is exempt" — why calls the quiet swallow ("memory is the whole asset for anyone who actually worked with the material") with the strike-or-limit-to-know-how ask and its own jump row. (2) Return-or-destroy on exit credited on the covenant finding ("hands the materials back when the deal ends — the fairer shape"); RET_RE runs document-wide since return sentences often drop "confidential". (3) Fair carve-outs credited (public domain, independently developed/acquired, already known), scanned only inside confidentiality-context segments so unrelated "publicly available" data notices stay silent. Why-assembly order now: direction, carve-outs, return, duration. Structural pin: "// Cycle #400 — polish: exits, memories, and fair carve-outs graded."
**Gate:** node --check ✓ · unit 490/490 · smoke 335 pass / 0 fail / 139 skipped · integration 16/16 in-suite, no flake. Standalone confidentiality block green first run.
**Prompt Intention:** Loop discipline held (polish of #399's lens). Next cycle #401 is ADD — scope-check candidates: splitting a single-regex trap topic into its own lens, or a non-lens workspace improvement. Scope-check before building.

## Cycle #401 — 2026-08-24 — ADD
**Changes Made:** Two changesets. (1) New license-grant lens (#23, 🎁 "License grants") — what you hand over when you upload. Scope-check: content/IP license grants rode on nothing but a doc-type hint string and a template sample — thinnest surface found yet (publicity covers only your name). detectLicense attributes the grantor ("you"/"members"/"consultants…" + hereby-grant shape), enumerates the breadth stack live from the document (worldwide / perpetual+irrevocable / royalty-free / sublicensable / exclusive / transferable — with lookbehinds so "non-exclusive" never counts as breadth), grades stated purpose (operate/improve/service = bounded-purpose credit; none = "read that as anywhere, for anything"), names moral-rights/attribution waivers as their own finding even without the word "license" (gate carries them), and flags post-deletion survival ("The license outlives your delete button"). Full house equipment: licBlock refs + analyze.html block 🎁 + guarded call site + cheat-sheet "License grants (what you hand over)" + ask-list "Narrow the license" + landing chip #23. (2) Harness root-fix: dark-mode zero-console-error test created its own bare browser context riding the live CDN proxy — the exact hole closed for the shared context in Cycle #388, reopened by this one fresh-context site. Extracted newHermeticContext(opts) helper (serviceWorkers block + once-guarded SRI-preserving CDN cache + external-request fulfillment) and converted ALL ~80 browser.newContext sites suite-wide, so every context is hermetic and per-test route mocks layer cleanly on top (later routes win). Reproduced standalone first: four 504s through the flapping proxy; green immediately after conversion.
**Gate:** node --check ×2 ✓ · unit 490/490 · smoke 336 pass / 0 fail / 138 skipped · integration 16/16 in-suite, no flake.
**Prompt Intention:** Loop discipline held (add after #400 polish). Lens family now 23 surfaces. Next cycle #402 is POLISH of the license-grant lens — candidates: to-you revocable-license direction grading ("their permission can be yanked back"), trademark/name-use carve-outs credited, or parked items.

## Cycle #402 — 2026-08-24 — POLISH
**Changes Made:** License-grant lens graded in both directions. (1) The to-you grant is no longer invisible: TO_YOU_RE catches "grants you / grants to you / licenses to you" shapes (the standard "Company grants you a limited, non-exclusive, non-transferable, revocable license" clause), and when that same sentence drafts the permission as revocable/non-transferable/limited, a fourth finding fires — "Their permission can be yanked back" — explaining that access and everything living only inside the service ends on their say-so, with the export-path ask (data leaves in a usable format before access does). GRANT_THEM stays silent on these so the finding speaks alone. (2) Marketing named as a purpose: MARKETING_RE (marketing/promotional/advertising purposes, or "to promote" service/product/brand/platform) upgrades the purpose bit of the breadth finding from the generic "anywhere, for anything" warning to "your work can land in their ads", while bounded-purpose credit and the live breadth enumeration stay intact beside it. Smoke: two new corpora — revocDoc (items exactly 1, /yanked back/, /export path/) and mktDoc (/land in their ads/ + worldwide enumeration still lands) — plus structural pin for the #402 marker.
**Gate:** node --check ✓ · license-grants smoke block green standalone · full gate npm run check ✓ (unit 490, smoke 336, integration 16).
**Prompt Intention:** Loop discipline held (polish after #401 add). Lens family steady at 23 surfaces. Next cycle #403 is ADD — scope-checking candidates first: trademark/name-and-logo use carve-outs as a credit inside this lens vs. splitting another single-regex trap topic into a dedicated lens.
