(function(){
  ...
  function analyzePage(){
    ...
    function buildQuestionsList(raw, ctx){
      ...
      const tal = { r: 0, a: 0, g: 0 };
      (lastFlags || []).forEach(f => { tal[f.rule.sev] = (tal[f.rule.sev] || 0) + 1; });
      // Per detected risk, one question + severity weight
      (lastFlags || []).slice(0, 6).forEach(f => {
        const key = f.rule.label.toLowerCase();
        const sev = f.rule.sev;
        let q = '';
        if(key.indexOf('non-refundable') >= 0){
          q = 'Why is this fee non-refundable? What conditions would change the refund policy?';
        } else if(key.indexOf('auto') >= 0){
          q = 'What does the cancellation / opt-out window look like? Can I switch to month-to-month?';
        } else if(key.indexOf('indemn') >= 0){
          q = 'Is the indemnification mutual? Can the cap be set at the value of the contract?';
        } else if(key.indexOf('arbitration') >= 0 || key.indexOf('jury') >= 0){
          q = 'Can we keep individual arbitration optional, not mandatory? Where is the venue?';
        } else if(key.indexOf('class') >= 0){
          q = 'Is there a way to keep the right to a class action if this clause is unenforceable in our state?';
        } else if(key.indexOf('late') >= 0 || key.indexOf('penalty') >= 0){
          q = 'Can we cap late fees at 5% of the overdue amount, in line with federal credit-card standards?';
        } else if(key.indexOf('sole discretion') >= 0){
          q = 'Can we add a 30-day notice requirement so changes can\'t ambush us?';
        } else {
          q = 'Can we revise the ' + f.rule.label.toLowerCase() + ' clause to be more balanced? What specifically concerns us is: ' + f.rule.why;
        }
        out.push({ q: q, sev: sev });
      });
      // Tone-based questions
      const tone = (typeof analyzeTone === 'function') ? analyzeTone(raw) : null;
      if(tone && tone.pressure > 60){
        out.push({ q: 'Several clauses feel one-sided — can we tone down the "shall" / "must" / "immediately" language?', sev: 'a' });
      }
      if(tone && tone.clarity < 60){
        out.push({ q: 'Some language is hard to parse — could we add a short "definitions" section?', sev: 'g' });
      }
      ...
      if(mletter === 'D' || mletter === 'F'){
        out.push({ q: 'Maturity is below 65/100 — what\'s the single most important fix we should request before signing?', sev: 'r' });
      } else if(mletter === 'B' || mletter === 'C'){
        out.push({ q: 'Maturity is mid-grade — which clauses should we ask your team to tighten up before signing?', sev: 'a' });
      }
      ...
      if(jl && jl.textContent.trim() && jl.textContent.toLowerCase().indexOf('state of') < 0){
        out.push({ q: 'Venue is currently ' + jl.textContent.trim() + ' — can we change it to our home state?', sev: 'a' });
      }
      // Dedupe + sort by severity (r → a → g) + cap at 8
      const seen = new Set();
      const dedup = out.filter(o => { if(seen.has(o.q)) return false; seen.add(o.q); return true; });
      dedup.sort((a, b) => {
        const rank = { r: 0, a: 1, g: 2 };
    ...
    function renderPrioBlock(raw, ctx){
      ...
      const html = cells.map(c => {
        ...
        const all = (p.lists[c.ki] || []);
        const top = all.slice(0, 2).join(' · ');
        const full = all.join(', ');
        return '<div class="prio-cell' + cls + '" style="grid-column:' + (c.x + 2) + ';grid-row:' + (c.y + 1) + '" data-prio-ki="' + esc(c.ki) + '" data-prio-label="' + esc(meta.label) + '" data-prio-full="' + esc(full) + '" title="Click to see all risks in this cell">' +
      ...
      // Iter #169 — copy as markdown + zoom cell
      const controls = '<div class="prio-controls">' +
        '<span class="prio-count">' + total + ' risk' + (total === 1 ? '' : 's') + '</span>' +
        '<button type="button" class="prio-md ghost-btn ghost-btn-sm" id="prioCopyMdBtn" title="Copy the matrix as a markdown table">📋 copy as markdown</button>' +
      '</div>';
      prioMatrix.innerHTML = header + '<div class="prio-grid">' + html + '</div>' + controls;
      prioBlock.hidden = false;
      if(prioNote){
        prioNote.innerHTML = '<span class="riskNote-lead">' + total + ' risk' + (total === 1 ? '' : 's') + ' plotted by impact × likelihood</span> · ' +
          'Document jargon density (' + (p.likelihood) + ' likelihood) drives the x-axis. <b>Top-left (high impact + likely) = address first.</b> Click any cell to see the full list of risks inside it.';
      }
      // Iter #169 — click-to-zoom
      $$('.prio-cell', prioMatrix).forEach(cell => {
        cell.addEventListener('click', () => {
          const label = cell.getAttribute('data-prio-label') || '';
          const full = cell.getAttribute('data-prio-full') || '';
          if(typeof showAnalyzeToast === 'function'){
            showAnalyzeToast('🔍 ' + label + ': ' + (full || 'no risks'));
          }
        });
      });
      // Iter #169 — copy as markdown
      const mdBtn = document.getElementById('prioCopyMdBtn');
      if(mdBtn){
        mdBtn.addEventListener('click', async () => {
          let md = '# Risk priority matrix\n\nGenerated by ClearDoc on ' + new Date().toLocaleString() + '\n\n' +
            '| Impact ↓ / Likelihood → | Low | Mid | High |\n' +
            '|---|---|---|---|\n' +
            '| **High** | ' + (p.counts['high|low'] || 0) + ' | ' + (p.counts['high|mid'] || 0) + ' | ' + (p.counts['high|high'] || 0) + ' |\n' +
            '| **Mid** | ' + (p.counts['mid|low'] || 0) + ' | ' + (p.counts['mid|mid'] || 0) + ' | ' + (p.counts['mid|high'] || 0) + ' |\n' +
            '| **Low** | ' + (p.counts['low|low'] || 0) + ' | ' + (p.counts['low|mid'] || 0) + ' | ' + (p.counts['low|high'] || 0) + ' |\n\n';
          Object.keys(p.groups).forEach(k => {
            if(p.counts[k] > 0) md += '- **' + p.groups[k].label + '** (' + p.counts[k] + '): ' + (p.lists[k] || []).join(', ') + '\n';
          });
          let copied = false;
          try { if(navigator.clipboard) { await navigator.clipboard.writeText(md); copied = true; } }
          catch(_){ /* fall through */ }
          if(!copied){
            try {
              const ta = document.createElement('textarea');
          ...
          if(typeof showAnalyzeToast === 'function') showAnalyzeToast(copied ? '📋 Matrix copied as markdown' : '⚠ Couldn’t copy');
          mdBtn.textContent = copied ? '✓ copied' : '📋 copy as markdown';
          setTimeout(() => { if(mdBtn.isConnected) mdBtn.textContent = '📋 copy as markdown'; }, 2500);
        });
      }
    }
    ...
    function renderBoardBlock(raw, ctx){
      ...
      // Iter #167 — markdown export
      const mdBtn = document.getElementById('boardCopyMdBtn');
      if(mdBtn){
        mdBtn.addEventListener('click', async () => {
          let md = '# Negotiation strategy board\n\nGenerated by ClearDoc on ' + new Date().toLocaleString() + '\n\n';
          COLUMNS.forEach(col => {
            const colItems = items.filter(i => i.col === col.key);
            md += '## ' + col.label + ' (' + colItems.length + ')\n\n';
            if(colItems.length){
              md += '| Risk | Sample | Counter-clause |\n|---|---|---|\n';
              colItems.forEach(it => {
                md += '| ' + (it.label || '?').replace(/\|/g, '\\|') + ' | ' + (it.sample || '').slice(0, 80).replace(/\|/g, '\\|').replace(/\n/g, ' ') + ' | ' + (it.counter || '').slice(0, 120).replace(/\|/g, '\\|').replace(/\n/g, ' ') + ' |\n';
              });
              md += '\n';
            } else {
              md += '_No items._\n\n';
    ...
    function renderVersionBlock(raw, ctx){
      ...
      buildVersionDelta(raw, ctx).then(d => {
        ...
        // Iter #145 polish — changelog copy + save-as-baseline chips
        let changelog = '';
        if(d.prev){
          changelog = '# Document analysis changelog\n' +
            'Generated by ClearDoc on ' + new Date().toLocaleString() + '\n\n' +
            '• Maturity: ' + d.cur.letter + ' (' + d.cur.num + '/100) — was ' + d.prev.letter + ' (' + d.prev.num + '/100)\n' +
            '• Total risks: ' + d.cur.risk + ' — was ' + d.prev.risk + '\n' +
            (d.exposureDelta != null ? '• Exposure: $' + (d.cur.exposure || 0).toLocaleString('en-US') + ' — was $' + (d.prev.exposure || 0).toLocaleString('en-US') + ' (Δ $' + d.exposureDelta + ')\n' : '') +
            '\nFingerprint: ' + (d.last.fp || '—') + '\n' +
            'Compared against: ' + new Date(d.prev.ts).toLocaleString() + '\n';
        }
        const controls = '<div class="version-controls">' +
          '<span class="version-save-count">' + ((JSON.parse(localStorage.getItem('cleardoc:receipt-log') || '[]') || []).length) + ' receipts saved</span>' +
          '<button type="button" class="ghost-btn ghost-btn-sm" id="versionCopyChangelog" title="Copy the changelog as plain text">📋 copy changelog</button>' +
          '<button type="button" class="ghost-btn ghost-btn-sm" id="versionSaveBaseline" title="Save this analysis as a new baseline for future comparisons">💾 save as baseline</button>' +
        '</div>';
        versionGrid.innerHTML = cells.join('') + controls;
        ...
        // Iter #145 polish — copy changelog
        const copyBtn = document.getElementById('versionCopyChangelog');
        if(copyBtn){
          copyBtn.addEventListener('click', async () => {
            let copied = false;
            try {
              if(navigator.clipboard) { await navigator.clipboard.writeText(changelog); copied = true; }
            } catch(_){ /* fall through */ }
            if(!copied){
              try {
                const ta = document.createElement('textarea');
                ta.value = changelog; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta);
                copied = true;
              } catch(_){ /* ignore */ }
            }
            if(typeof showAnalyzeToast === 'function') showAnalyzeToast(copied ? '📋 Changelog copied' : '⚠ Couldn’t copy');
            copyBtn.textContent = copied ? '✓ copied' : '📋 copy changelog';
            setTimeout(() => { if(copyBtn.isConnected) copyBtn.textContent = '📋 copy changelog'; }, 2500);
          });
        }
        // Iter #145 polish — save as new baseline
        const saveBtn = document.getElementById('versionSaveBaseline');
        if(saveBtn){
          saveBtn.addEventListener('click', () => {
            try {
              const stored = JSON.parse(localStorage.getItem('cleardoc:receipt-log') || '[]') || [];
              stored.unshift({
                ts: Date.now(),
                fp: d.cur.letter + '-' + d.cur.num + '-' + d.cur.risk + '-' + d.cur.exposure,
                length: d.cur.wordCount,
                excerpt: '',
              });
              while(stored.length > 50) stored.pop();
              localStorage.setItem('cleardoc:receipt-log', JSON.stringify(stored));
              if(typeof showAnalyzeToast === 'function') showAnalyzeToast('💾 Saved as new baseline');
              renderVersionBlock(raw, ctx);
            } catch(_){
              if(typeof showAnalyzeToast === 'function') showAnalyzeToast('⚠ Couldn’t save');
            }
          });
        }
      });
    ...
    function renderQuestionsBlock(raw, ctx){
      ...
      // Iter #127 — per-row "✓ got answer" persisted in localStorage
      const ANSWERED_KEY = 'cleardoc:questions-answered';
      let answered = {};
      try { answered = JSON.parse(localStorage.getItem(ANSWERED_KEY) || '{}') || {}; } catch(_){ answered = {}; }
      const answeredCount = qs.filter(o => answered[o.q]).length;
      const total = qs.length;
      const sevLabel = { r: 'trap', a: 'watch', g: 'note' };
      const sevClass = (s) => (s === 'r' ? 'trap' : s === 'a' ? 'watch' : 'note');
      quesList.innerHTML = qs.map((o, i) => {
        const isAnswered = !!answered[o.q];
      ...
      if(quesCopyBtn){
        quesCopyBtn.addEventListener('click', async () => {
          const text = qs.map((o, i) => (i + 1) + '. ' + o.q).join('\n\n');
          let copied = false;
          try {
            if(navigator.clipboard) { await navigator.clipboard.writeText(text); copied = true; }
          } catch(_){ /* fall through */ }
      ...
      $$('.ques-done', quesList).forEach(btn => {
        btn.addEventListener('click', () => {
          const q = btn.getAttribute('data-ques-done') || '';
          if(!q) return;
          let ans = {};
          try { ans = JSON.parse(localStorage.getItem(ANSWERED_KEY) || '{}') || {}; } catch(_){ ans = {}; }
          ans[q] = !ans[q];
          try { localStorage.setItem(ANSWERED_KEY, JSON.stringify(ans)); } catch(_){ /* quota */ }
    ...
    // iter #202: build a Markdown (.md) copy of the current analysis.
    // Same data sources as buildAnalysisSummary() but rendered with
    // ATX headings, fenced rewrite content, and severity tags so it
    // drops cleanly into Obsidian / Notion / email drafts.
    function buildAnalysisMarkdown(){
      ...
      if(plainOut && plainOut.textContent && plainOut.textContent.trim()){
        md.push('## Plain-English Rewrite');
        md.push('');
        md.push(stripHtmlToText(plainOut.innerHTML));
        md.push('');
        if(levelFrom && levelTo) md.push('_Reading level: '+levelFrom.textContent+' → '+levelTo.textContent+'_');
        if(jargonCount) md.push('_Jargon found: '+jargonCount.textContent+'_');
        md.push('');
      }
      // Risk radar
      if(lastFlags && lastFlags.length){
        ...
        md.push('');
        lastFlags.forEach((f,i)=>{
          md.push((i+1)+'. **['+f.rule.label.toUpperCase()+']**');
      ...
      if(deadlinesBlock && !deadlinesBlock.hidden && deadlinesList){
        const rows=deadlinesList.querySelectorAll('.deadline-row');
        if(rows.length){
          md.push('## Deadlines ('+rows.length+')');
          md.push('');
          rows.forEach(row=>{
            const d=row.querySelector('.deadline-date');
            const x=row.querySelector('.deadline-desc');
            md.push('- '+(d?d.textContent.trim():'')+' — '+(x?x.textContent.trim():''));
          });
          md.push('');
        }
      }
      // iter #202 v2: Next Steps — mirrors the .txt buildAnalysisSummary()
      // section so the .md and .txt exports stay in sync.
      if(nextStepsBlock && !nextStepsBlock.hidden && nextStepsList){
        const steps=nextStepsList.querySelectorAll('li');
        if(steps.length){
          md.push('## Next Steps');
          md.push('');
          steps.forEach((s,i)=>{ md.push((i+1)+'. '+s.textContent.trim()); });
          md.push('');
        }
      }
      // iter #202 v2: Response Draft — the email reply the analyzer
      // already composed for the user; included so the .md export is
      // truly "send-and-go" without anyone re-copying from elsewhere.
      if(draftOut && draftOut.value && draftOut.value.trim()){
        md.push('## Response Draft');
        md.push('');
        md.push('```');
        md.push(draftOut.value.trim());
        md.push('```');
        md.push('');
      }
      md.push('---');
      md.push('_Generated by [ClearDoc](https://cleardoc.app) — '+new Date().toISOString().slice(0,10)+'_');
      md.push('');
      md.push('**NOT LEGAL ADVICE.** For high-stakes documents, consult a qualified attorney.');
      return md.join('\n');
    }
    function exportAnalysisMd(){
      if(!lastRaw){
        if(msg){msg.textContent='Analyze a document first, then export as Markdown.'; msg.className='analyze-msg';}
        return;
      }
      const text=buildAnalysisMarkdown();
      const stamp=new Date().toISOString().slice(0,10);
      const filename='cleardoc-analysis-'+stamp+'.md';
      try{
    ...
    async function copyAnalysisMd(){
      if(!lastRaw){
        if(msg){msg.textContent='Analyze a document first, then copy as Markdown.'; msg.className='analyze-msg';}
        return;
      }
      const text=buildAnalysisMarkdown();
      ...
      try{
        ...
        console.warn('[copy-md] clipboard failed',e);
      }
      if(btn) flashButton(btn, ok ? 'Copied ✓' : 'Copy failed', ok ? 1400 : 1800);
    }
    // iter #210: build a short email summary (subject + plain-text
    // body) for forwarding to a lawyer. Picks the verdict label,
    // the top-concern quote, and the top 3 risks with their
    // counter-suggestions. Subject / body shape mirrors the
    // existing email-draft builder (line ~9879) so the two share
    // a recognizable format.
    function buildEmailSummary(){
      const order = { r:0, a:1, g:2 };
      const top = (lastFlags && lastFlags.length)
        ? lastFlags.slice().sort((x,y)=> (order[(x&&x.rule&&x.rule.sev)]??3) - (order[(y&&y.rule&&y.rule.sev)]??3))[0]
        : null;
      const vLabel = verdictDisplay && verdictDisplay.querySelector && verdictDisplay.querySelector('.verdict-label');
      const vSummary = verdictDisplay && verdictDisplay.querySelector && verdictDisplay.querySelector('.verdict-summary');
      const verdictText = (vLabel && vLabel.textContent.trim()) || '(no verdict)';
      const summaryText = (vSummary && vSummary.textContent.trim()) || '';
      const subject = 'Document analysis — verdict: ' + verdictText + ' (cleardoc.app)';
      const lines = [];
      lines.push('Hi,');
      lines.push('');
      ...
      if(top && top.rule){
        ...
        const q = trunc(top.s || '', 240).replace(/\s+/g,' ').trim();
        lines.push('"' + q + '"');
      ...
      const sorted = (lastFlags || []).slice().sort((x,y)=> (order[(x&&x.rule&&x.rule.sev)]??3) - (order[(y&&y.rule&&y.rule.sev)]??3)).slice(0, 3);
      sorted.forEach((f, i) => {
        lines.push((i+1) + '. [' + ((f.rule && f.rule.label) || '').toUpperCase() + ']');
        lines.push('   "' + trunc(f.s || '', 200).replace(/\s+/g,' ').trim() + '"');
        if(f.rule && f.rule.why) lines.push('   Why: ' + f.rule.why);
        if(f.rule && f.rule.counter) lines.push('   Counter: ' + f.rule.counter);
        lines.push('');
      });
      lines.push('---');
      // iter #210 v2: include the analyzed-at timestamp + the source
      // filename (if the user attached a file) so the recipient has
      // context for WHEN the analysis ran + WHAT it was applied to.
      // iter #211 v2: also include the document fingerprint so the
      // email is self-identifying about which document was analyzed
      // (the recipient can compare against the on-page chip on a
      // re-analysis to confirm they're looking at the same input).
      const _emTs = (_analyzedState && _analyzedState.ts) ? _analyzedState.ts : Date.now();
      lines.push('Analyzed: ' + new Date(_emTs).toLocaleString());
      if(_fpState && _fpState.short){
        lines.push('Document fingerprint: #' + _fpState.short + ' (full SHA-256 on request)');
      }
      if(attachedFile && attachedFile.name) lines.push('Source file: ' + attachedFile.name);
      lines.push('Generated by cleardoc.app');
      lines.push('NOT LEGAL ADVICE — review with a qualified attorney.');
      return { subject: subject, body: lines.join('\n') };
    }
    function openEmailSummary(){
      if(!lastRaw){
        if(msg){msg.textContent='Analyze a document first, then email the summary.'; msg.className='analyze-msg';}
        return;
      }
      const draft = buildEmailSummary();
      const href = 'mailto:?subject=' + encodeURIComponent(draft.subject) + '&body=' + encodeURIComponent(draft.body);
      try {
        window.location.href = href;
      } catch(_) {
        // Some browsers block programmatic location changes; fall back
        // to opening a temporary anchor.
        const a = document.createElement('a');
        a.href = href;
    ...
    // iter #210 v2: copy-to-clipboard sibling for users without a
    // configured mail client (or who want to paste into chat / a
    // ticket instead of email). Uses the same navigator.clipboard +
    // execCommand fallback pattern as the existing copyAnalysisMd
    // handler.
    async function copyEmailSummary(){
      if(!lastRaw){
        if(msg){msg.textContent='Analyze a document first, then copy the email summary.'; msg.className='analyze-msg';}
        return;
      }
      const draft = buildEmailSummary();
      const text = 'Subject: ' + draft.subject + '\n\n' + draft.body;
      ...
      try{
        ...
        console.warn('[copy-email] clipboard failed',e);
      }
      if(btn) flashButton(btn, ok ? 'Copied ✓' : 'Copy failed', ok ? 1400 : 1800);
    }
    ...
    // iter #217: build a risk checklist in Markdown checkbox format
    // so users can paste directly into Jira, Linear, Notion, or any
    // task manager. Each risk becomes ☐ [label] — risk sentence.
    // Traps (r) get ⛔ emoji, watches (a) get ⚠, notes (g) get ℹ.
    // Risks are sorted by severity (traps → watches → notes) so the
    // most critical items appear first — the same order the threat
    // score uses internally (trap=10, watch=4, note=1).
    // The checklist header includes the threat level and score when
    // available, giving anyone reading the pasted list immediate
    // context about the document's overall risk posture.
    function buildRiskChecklist(){
      if(!lastFlags || !lastFlags.length) return '';

      // Stable sort: traps (0) → watches (1) → notes (2),
      // preserving document order within each tier via index.
      const sevOrder = { r:0, a:1, g:2 };
      const flags = lastFlags.filter(f => f && f.rule).slice();
      flags.sort((a,b) => {
        const diff = (sevOrder[a.rule.sev] ?? 2) - (sevOrder[b.rule.sev] ?? 2);
        return diff !== 0 ? diff : (a.i ?? 0) - (b.i ?? 0);
      });

      const lines = ['# Risk checklist', ''];

      // Include threat level header when available — mirrors the
      // on-page threat score pill so the pasted checklist reads
      // the same as what the user sees on screen (iter #216).
      if(typeof computeThreatScore === 'function' && typeof threatScore !== 'undefined' && !threatScore.hidden){
        const t = computeThreatScore(lastFlags);
        lines.push('**Threat level: ' + t.score + ' (' + t.level + ')** · ' + t.traps + ' trap' + (t.traps===1?'':'s') + ' · ' + t.watches + ' watch' + (t.watches===1?'':'es') + ' · ' + t.notes + ' note' + (t.notes===1?'':'s'));
        lines.push('');
      }

      flags.forEach(f => {
        const emoji = f.rule.sev === 'r' ? '⛔' : f.rule.sev === 'a' ? '⚠' : 'ℹ';
        const priority = f.rule.sev === 'r' ? '[P0] ' : f.rule.sev === 'a' ? '[P1] ' : '[P2] ';
        lines.push('- ☐ ' + emoji + ' ' + priority + '**' + esc(f.rule.label || 'Risk') + '** — ' + esc(f.s));
      });
      lines.push('');
      lines.push('*Generated by ClearDoc · review and assign each item before signing.*');
      return lines.join('\n');
    }
    async function copyAnalysisChecklist(){
      if(!lastFlags || !lastFlags.length){
        if(msg){msg.textContent='No risks found — copy a checklist when the analysis has flagged items.'; msg.className='analyze-msg';}
        return;
      }
      const text = buildRiskChecklist();
      let ok=false;
      try{
        if(navigator.clipboard && navigator.clipboard.writeText){
        ...
        console.warn('[copy-checklist] clipboard failed',e);
      }
    ...
    if(input){
      ...
      if(receiptBtn) receiptBtn.hidden = false;
      ...
      // Iter #110: receipt / proof pack — package the analysis
      // (timestamp + doc snippet + scorecard + SHA-256 fingerprint
      // of the analyzed text) into a printable "I reviewed this
      // on [date]" receipt. Pure local — uses crypto.subtle for
      // the fingerprint, no network calls.
      async function sha256Hex(str){
        try {
          if(typeof crypto === 'undefined' || !crypto.subtle) return 'fingerprint-unavailable';
          const enc = new TextEncoder().encode(str);
          const buf = await crypto.subtle.digest('SHA-256', enc);
          const arr = Array.from(new Uint8Array(buf));
          return arr.map(b => b.toString(16).padStart(2,'0')).join('');
        } catch(_){ return 'fingerprint-unavailable'; }
      }
      const receiptBtnEl = document.getElementById('receiptBtn');
      if(receiptBtnEl){
        receiptBtnEl.addEventListener('click', async () => {
          if(typeof showConfirmModal !== 'function') return;
          const raw = (typeof input !== 'undefined' && input && (input.value || '').trim()) || '';
          if(!raw){
            if(typeof showAnalyzeToast === 'function') showAnalyzeToast('⚠ Nothing to receipt — paste a document first');
            return;
          }
          const docSnippet = raw.slice(0, 280);
          ...
          const maturity = (function(){
            const v = maturityGrid && maturityGrid.querySelector('.mat-letter-glyph');
            const n = maturityGrid && maturityGrid.querySelector('.mat-letter-num');
            return (v && n) ? (v.textContent.trim() + ' (' + n.textContent.replace(/[^0-9]/g,'') + '/100)') : '—';
          })();
          const exposure = (function(){
            const t = riskDetail && riskDetail.textContent || '';
            const m = t.match(/worst-case exposure[^A-Z]*\$[0-9,]+/i);
          ...
          const fpShort = fp.slice(0, 16);
          const fpTail = fp.slice(-16);
          const stamp = new Date().toISOString();
          const html =
            '<div class="receipt-modal">' +
              '<div class="receipt-stamp">' +
                '<div class="receipt-checkmark">✓</div>' +
                '<div class="receipt-stamp-meta">' +
                  '<h2>Document Review Receipt</h2>' +
                  '<div class="receipt-sub">Generated by ClearDoc · Stamp ' + esc(stamp) + '</div>' +
                '</div>' +
              '</div>' +
              '<div class="receipt-row"><b>Document length</b><span>' + docLen.toLocaleString('en-US') + ' characters</span></div>' +
              '<div class="receipt-row"><b>First 280 chars</b><span style="font-family:JetBrains Mono,monospace;font-size:11px">' + esc(docSnippet) + (raw.length > 280 ? '…' : '') + '</span></div>' +
              '<div class="receipt-row"><b>Maturity grade</b><span>' + esc(maturity) + '</span></div>' +
              '<div class="receipt-row"><b>Worst-case exposure</b><span>' + esc(exposure) + '</span></div>' +
              '<div class="receipt-row"><b>Governing law</b><span>' + esc(juris) + '</span></div>' +
              '<div class="receipt-row"><b>Risks flagged</b><span>' + tal.r + ' trap · ' + tal.a + ' watch · ' + tal.g + ' note</span></div>' +
              '<div class="receipt-row" style="flex-direction:column;align-items:stretch"><b>SHA-256 fingerprint</b>' +
                '<div class="receipt-fingerprint">' + esc(fp) + '<br><span style="font-size:10px">' + esc(fpShort) + '…' + esc(fpTail) + '</span></div>' +
              '</div>' +
              '<div class="receipt-sign-row">' +
                '<div><div class="receipt-sign-cell">&nbsp;</div><div class="receipt-sign-cap">Reviewed by (signature)</div></div>' +
                '<div><div class="receipt-sign-cell">' + esc(new Date().toLocaleString()) + '</div><div class="receipt-sign-cap">Date reviewed</div></div>' +
              '</div>' +
          ...
          setTimeout(() => {
            ...
            if(copyBtn){
              copyBtn.addEventListener('click', async () => {
                try {
                  if(navigator.clipboard) await navigator.clipboard.writeText(fp);
                  else throw new Error('no-clip');
                } catch(_){
                  const ta = document.createElement('textarea');
                  ta.value = fp; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta);
                }
                if(typeof showAnalyzeToast === 'function') showAnalyzeToast('📋 Fingerprint copied');
                copyBtn.textContent = '✓ copied!';
                setTimeout(() => { if(copyBtn.isConnected) copyBtn.textContent = '📋 copy fingerprint'; }, 2500);
              });
            }
          }, 50);
        });
      ...
      if(cheatBtn){
        cheatBtn.addEventListener('click', () => {
          ...
          const html =
            '<div class="cheat-sheet-modal">' +
              ...
              '<div class="cheat-actions">' +
                ...
                '<button type="button" class="ghost-btn cheat-btn" id="cheatEmailBtn">✉ email this</button>' +
              ...
              '</div>' +
              '<div class="cheat-footer">Filename: <code>cleardoc-cheatsheet-' + esc(new Date().toISOString().slice(0,10)) + '.pdf</code> · Generated locally on this device.</div>' +
            '</div>';
          showConfirmModal({
          ...
          setTimeout(() => {
            ...
            const emailBtn = document.getElementById('cheatEmailBtn');
            if(emailBtn){
              emailBtn.addEventListener('click', () => {
                const subject = 'Negotiator cheat sheet (ClearDoc)';
                const body = 'NEGOTIATOR CHEAT SHEET — ' + new Date().toLocaleString() + '\n' +
                  'Maturity: ' + (maturity || '—') + '\n' +
                  'Worst case: ' + (exposure || '—') + '\n' +
                  'Venue: ' + (jurisdiction || '—') + '\n' +
                  'Risk tally: ' + (tally.r + 'r/' + tally.a + 'a/' + tally.g + 'g') + '\n\n' +
                  'See the full printable cheat sheet on the page where you clicked this.\n\n' +
...
if(comparePanel.hidden){compareVerdict&&(compareVerdict.hidden=true);compareStats.innerHTML='';return;}
    ...
    // pre-filled subject + body carrying the verdict, the top
    // concern, and the top 3 risks with counter-suggestions. Same
    // subject/body shape the existing email-draft builder uses so
    // the two paths share a recognizable format.
